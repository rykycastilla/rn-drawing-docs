  ...
  return (
    <section id="tools" className="py-20 bg-background">
      <div className="container">
        <h2 className="text-3xl font-bold text-center mb-10">Todas las Herramientas</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
          <ToolCard
            icon={ ZoomIn }
            name="Zoom"
            description="Acerca o aleja el lienzo para trabajar en detalles" />
          <ToolCard icon={ Pipette } name="Eye Dropper" description="Captura cualquier color del lienzo" />
          <ToolCard
            icon={ SprayCanIcon }
            name="Spray"
            description="Crea efectos de aerosol con partículas personalizables" />
          <ToolCard icon={ Pen } name="Pen" description="Trazos fluidos con presión variable" />
          <ToolCard
            icon={ TapeIcon as unknown as LucideIcon }
            name="Tape"
            description="Crea líneas rectas y formas con cinta adhesiva" />
          <ToolCard icon={ Pencil } name="Pencil" description="Trazo natural con textura de lápiz" />
          <ToolCard icon={ Eraser } name="Eraser" description="Borra con precisión y control total" />
          <ToolCard
            icon={ PaintBucket }
            name="Filler"
